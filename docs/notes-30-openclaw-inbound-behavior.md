...
### 根因（双通道 + 限流）
收到消息时**同时调用**了两个独立通道：
1. `queueInitialAckReaction({ adapter })` → **裸加 👀**（不经状态机，`DEFAULT_ACK_REACTION="👀"`）
2. `statusReactions.setQueued()` → ⏳（走状态机）

后果：
- 👀 裸加后**不在状态机的 activeEmojis 里** → 替换式清不掉 → 永远挂着（"等待时是眼睛"）
- reaction 操作**翻倍**（👀+⏳ 双加 + 后续替换）→ 触发 Discord reaction 限流 → **后续 🧠/🛠️ 全部静默失败**（"思考时没标签"）

### 修复
删掉 `queueInitialAckReaction` 调用，**只走状态机单通道**：
`收到 → ⏳ → 👀(开工) → 🧠(思考) → 🛠️(工具) → ✅(完成,清空)`

### 经验
- **多条状态通道叠加 = 表情幽灵 + 限流**——状态必须单通道管理（controller 统一负责增删）
- 裸 API 调用绕过状态机 → 状态机失去控制权（清不掉）
